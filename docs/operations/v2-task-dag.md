...
- **G1**（v1、M0〜M4に適用。M5以降は下のv2）: 当該milestoneの全実装・設計taskがcomplete。
  1件でも未completeならgateはblockedのまま。
- **G1**（v2、**M5以降にのみ適用**）: 当該milestoneが守ると宣言した**性質の集合P1..Pn**が
  すべて充足されている。各性質には「どのevidenceがそれを充足するか」が紐づく。
  **task一覧は手段であって合格条件ではない。性質に紐づかないtaskはgateを止めない。**
  性質リストには次を必ず含める——**domainがモデル化した遷移に、test以外の到達経路がある。**
  v1からv2への変更理由と、なぜM0〜M4を再計算しないかは§12.1にある。
...
## 12. 残タスクが減らない構造への対処（2026-08-26、規約変更）

**この節は遡及しない。既存のevidenceを再観測させない。** 以下の規則は制定時点以後の
観測にのみ適用する。

### 12.0 何を測って、この変更に至ったか
...
| fan-out | **≈1.0**（残数が動かない） |
| 未完了のうちpacket完備 | **4/21** |
| 当初計画（§2のDAG表 V2-006〜V2-039） | 約40件 |
| 現在のtask総数 | 96件 |

**残り56件はすべてこの差から出たremediationである。** 個々の発見は健全だったが、
**発見が自動的に到達不能化に変換される機構**があった。以下はその機構を止める。

### 12.1 A: G1をtask集合から性質集合へ（G1 v2）

**これが正のフィードバックの結節点だった。** 旧G1は「当該milestoneの全taskがcomplete」を
要求する。するとtaskが1件増えるたびにgateの合格条件が自動的に厳しくなり、
**健全な発見がそのまま到達不能化に変換される。** 発見すれば遠ざかる構造では、
測る動機と閉じる動機が正面から衝突する。

**G1 v2はgateの合格条件を性質の集合で定義する。** 各性質に充足evidenceを紐づけ、
task一覧は手段として扱う。性質に紐づかないtaskはgateを止めない。

性質リストには**「domainがモデル化した遷移に、test以外の到達経路がある」を必ず含める。**
理由は測定である。M5 remediation群のoutcomeは全部同じことを言っていた——
V2-086「no code outside a test constructs it」、V2-091「nothing can reach because the
pass has no caller」、V2-093「the only thing that drives one today is a test」。
**9 taskへの分割は「1 task 1境界」という規約の産物であって、問題の構造ではない。**

**適用範囲はM5以降のみ。M0〜M4のpassed判定は再計算しない。** 再計算すれば、この節が
止めようとしている遡及そのものを、この節の制定行為が実演することになる。

### 12.2 B: gate依存の凍結

**gate taskの`dependencies`はgate task作成時に確定し、以後の追加を禁止する。**
後から見つかったtaskはgateの**後ろ**に置く。

これは既に人力で毎回やっている操作を規約にするだけである。測定: **V2-018はdeps 1→6に
伸びた末にfailed。V2-080はdeps 4→17に膨らみ、手で9に戻して初めてrunningになった。**
gateの依存が後から伸びるかぎり、gateは定義上決して満たされない。

### 12.3 C: scope boundaryの改訂を認める（§9.4の改訂）

§9.4「scope boundaryは編集の**前**に書く」は維持する。そのうえで:

> **境界の改訂は禁止しない。** 改訂理由・改訂時刻・改訂前の表をevidence recordに残せば、
> **同一task内で境界を広げてよい。**

理由: 現状、境界外の発見に対する出口が「据え置く＝新task」しかないため、
**fan-outが構造的に1を切れない。** §9.4が防ぎたかったのは事後合理化による整合の詐称で
あって境界の不変性ではなく、**改訂履歴を残せば詐称は防げる**——事前の表と事後の表の
両方が記録に残り、いつ何を理由に広げたかが読めるからである。

Work OrderのA1テンプレートには次を書く: 「境界を広げる必要が生じたら、改訂前の表・改訂時刻・
改訂理由を記録したうえで広げてよい。新しいtaskを提案するのではなく、この手順を使うこと。」

### 12.4 D: 判定規則の遡及禁止

**gate規則G1〜G7と§8.xに版番号を付け、evidenceは観測時点の規則版で判定する。**
新設・改訂された規則は以後の観測にのみ適用する。
**既存のlive evidenceを、規則が変わったことを理由に再観測しない。**

| 規則 | 版 | 制定 |
|---|---|---|
| G1〜G5 | v1 | 初版 |
| G6 | v1 | 2026-08-25 |
| G6a / G6b（G6の分割） | v2 | 2026-08-25 |
| G7 | v1 | 初版 |
| §8.1（完了条件の読み方） | v1 | 2026-08-26（M5失敗を受けて） |
| §8.2 / §8.3 | v1 | 2026-08-26（M5再失敗を受けて） |
| **G1 v2（性質集合）** | **v2** | **2026-08-26（本節）** |
| **§12（本節）** | **v1** | **2026-08-26** |

測定: **判定規則が失敗のたびに増え、過去のevidenceを遡って失効させていた。** その結果
「live記録の再観測」taskがV2-063・V2-075・V2-080の**3本**発生した。live観測は回数上限
つきで、**V2-075は12/12で枯渇した——金額は8.00 USDのうち0.444129しか使っていない。**
規則churnの費用が、有限で補充できない資源で払われていた。

### 12.5 E: 宣言を実装より広くしない

以後、`docs/product/**`と`docs/architecture/**`に**未実装の挙動を書かない。**
意図・計画は別ファイルへ分離する。

**既存docの全面改訂はしない。** それ自体が巨大な生成源になる。既存の差は§12.6の台帳に
記録するだけにとどめる。

### 12.6 F: 差分台帳を1回だけ作る

`.agents/v2/DECLARATION-GAP.md`に、`docs/product/**`と`docs/architecture/**`の主張を
列挙し、コードと突き合わせた差を記録する。

**制約: taskを1件も作らない。修正も1件もしない。1ファイルに収める。**
以後の発見は「新規task」ではなく**この台帳の消し込み**として扱う。

目的は**分母を1回だけ確定させること**である。これを飛ばすとA〜Eを全部やっても
「あと何回増えるのか」に答えが出ない。

### 12.7 G: remediation予算

**1 milestoneあたりのremediation task数の上限を5件とする。** 超えたら
**milestoneの定義そのものを疑う**判定を起こす。

測定: **M5はV2-026→V2-081→V2-095と2回失敗して再判定木が広がり続けたが、
それを異常として検知する仕組みが無かった。** 上限は「これ以上は個別の欠陥ではなく
milestoneの切り方が違う」という信号である。

### 12.8 M5の性質集合P1..P6（G1 v2の具体適用）

G1 v2は性質集合を要求するので、それが無ければ規則は空である。**M5はこれで判定する。**
判定するtaskはV2-092（`repair_of: V2-081`）で、そのdependenciesは§12.2により凍結済みである。

| | 性質 | 種類 | 充足するもの |
|---|---|---|---|
| **P1** | docsだけのdriftでもpromotionが止まる | 安全性 | **充足済み**。`internal/release/pipeline.go`の`VerifySource`がgate・store・routeより前に拒否する。assertionは`pipeline_test.go:101`（1 byte反転＋negative control）と`:60`（digestを合わせても拒否） |
| **P2** | 全機能Evidenceを持たないcandidateはStableにならない | 安全性 | **充足済み**。`internal/domain/release.go:98-102`が欠けているcapability名を挙げて`ErrEvidenceIncomplete`を返す。assertionは`release_test.go:230`と`:372`の閉包grid列挙、`:122`の12拒否class分離 |
| **P3** | **domainがモデル化した遷移に、test以外の到達経路がある** | 到達性 | **未充足**。V2-084（`CompleteFraming`とClaimのready→active）、V2-089（Claim拒否）、V2-090（pause/cancelと出口）、V2-091（reconcile passのwaiting/recovering、promotionの評価と完了、出荷Runnerのjourney driver） |
| **P4** | ownerが1ページを超えるBacklogを読める | 到達性 | **未充足**。`cap-backlog-visibility`。cursor拒否はV2-079が既に直しており、V2-095の再観測で反転する見込み |
| **P5** | ownerが質問を出して答えられる | 到達性 | **未充足**。`cap-human-input-request`。「capturedからneeds-inputに入れない」はV2-082のframingとV2-084の`CompleteFraming`が触る範囲 |
| **P6** | Repository登録とowner向け文書の主張が実挙動と一致する | 到達性 | **未充足**。`cap-repository-registration`と`cap-user-documentation`。V2-095 |
...
V2-022のdogfoodで失敗した11 capabilityのうち**7件は宣言によって繰り延べられている。**
§8.3のとおりこれは判断ではなく測定で、capability自身が宣言する外部systemから導出する。

| 繰り延べ先 | capability |
|---|---|
| **D1（Cloud Runを宣言）** | `cap-preview-operation`, `cap-stable-promotion`, `cap-loop-control`, `cap-loop-self-update` |
| **M6（codexとopencodeを宣言、当該machineで未認証）** | `cap-autonomous-resolution`, `cap-shared-resource-allocation`, `cap-provider-operation` |

**この7件はM5のgateを止めない。** 逆向きの拘束も§8.3のとおり効く——上の4つの
product gapをこちら側に移すことはできない。繰り延べは宣言から導出するもので、
「到達できなかった」という観察からは導出しない。

### 12.10 §12.7の予算をM5に当てた結果

M5のremediation taskは統合後に**V2-084・V2-089・V2-090・V2-091・V2-095の5件**である。
**上限はちょうど5件で、M5はその境界上にある。**

統合前は7件だった（V2-091/092/093が別々）。**つまり§12.7の予算は、統合しなければ
発火していた。** これは規則が事後に都合よく作られていないことの確認になる——
上限は先に決め、当てた結果が境界上に落ちた。

以後M5にremediation taskを1件でも足すなら、それは個別の欠陥ではなく
**milestoneの切り方を疑う判定**を起こす合図である。

### 12.11 taskのoutcomeがtestの主張を反転させたなら、改名を許す

V2-089が判断を仰いできた点で、規則を1つ足す。

`TestClaimAgainstACapturedParentStillSucceeds`はV2-084のA6が**測定として置いた**testで、
V2-089の仕事はまさにその極性を反転させることだった。V2-089は正しく反転させ、
**改名禁止の規則に従って名前を残した。** その結果、名前が本文の主張の**否定**を述べる
testが残った。

**改名を許す。** ただし条件つきで、条件は§12.3と同じ構造である。

> taskのoutcome自体がtestの主張を反転させた場合、そのtestを改名してよい。
> **旧名・新名・assertion単位のdiffの3つを記録すること。**

**改名禁止規則が防ぎたかったのは、消したassertionが新しい名前の陰に隠れることである。**
名前が本文の否定を述べているtestは、次に読む人に対する罠で、残して誰の得にもならない。
記録があれば隠蔽は防げる——これは§12.3が境界の改訂について述べたことと同じ論法である。

**逆向きの拘束**: assertionを1つでも消す改名は、記録があっても許されない。
