...
- **CI matrix**: `AGENTIC_LOOP_TEST_GROUP`は`tests/run-e2e.sh`の既定群を1群へ絞る。明示的な`--groups`がある場合はそちらを優先する。ローカルの共通入口では環境変数を設定せず全4群を実行し、CIでは固定matrixの各jobが1群ずつ同じ共通入口から実行する。
