...
同一リポジトリの診断はlockで直列化される。Codexはread-only sandboxでコードを調査し、open・closedの既存Issueを検索して重複を避ける。Issueには問題、根拠、期待状態、受け入れ条件、対象パスを記載し、作成時に `diagnosis`、`category:improvement`、`agent:queued` を同時に付ける。診断プロセスはコードを変更せず、worker数、lease、専用worktree、検証、secret guard、PR、CI、review、mergeは[Issueキュー運用](issue-queue.md)のSupervisorとworkerが担う。
