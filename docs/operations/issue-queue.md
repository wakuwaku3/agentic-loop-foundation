...
- **Running Issues**: Issue番号・title・`(phase: ...)`・`(scope: ...)`に加え、`(started: 開始epoch, elapsed: 経過秒)`・`(timeout_at: 上限到達epoch[、超過なら「超過」]。`worker_timeout_seconds=0`では非表示)`・`(heartbeat: 最終heartbeat epoch)`・`(lease_expires: 期限epoch[、期限切れなら「期限切れ」])`・`(worktree: path[、dirty/diverged、または「なし」])`・`(pr: #番号 state=... checks=...)`を、追加のGitHub呼び出しなしでlocal state（`workers/<issue>.started`・`.lease`・`.resume`、scope cache）から表示する。加えて、workerがlocalへ書くprogress marker（`workers/<issue>.progress`、`epoch\tstage\tseq`。stageはenumのみ）から `(stage: plan|exec|...)`・`(progress: Ns ago)`・`(health: healthy|stalled|timeout)`を表示する。`health`は`timeout`（`worker_timeout_seconds`超過）> `stalled`（最後の進行がstage別のstall閾値を超過）> `healthy`の順で、local stateのない他host所有Issueは`unknown`（「不明」）。stall閾値はstageによって2段階に分かれる（[ADR 0031](../decisions/0031-stage-stall-threshold-and-provider-progress-signal.md)）: `plan`/`exec`/`replan`（1回の長いprovider呼び出しに専念するstage）は`queue.provider_stall_seconds`（既定3600秒）、それ以外は`queue.stall_seconds`（既定300秒）。いずれも`0`でそのbandのstall判定を無効化できる。`status --format json`の`workers[]`は実際に適用された閾値を`stall_threshold_seconds`として返す。色分けはTTYかつ`NO_COLOR`未設定のときだけANSI、pipe/JSONは無色。progress markerはworkerがstage境界と自ホスト制御区間で書く（heartbeatは更新しない）。副シグナルにはworker logに加えてplan/exec stageの出力file（`issue-N-plan.txt`・`issue-N-result.txt`とその一時的な`.raw.*`/`.final.*`）のmtimeも採用し、本文は一切読まずprovider待ちの長考をstalledと誤判定しない。
...
- **自動回復中**: Supervisorの次回pollが自動的に処理する異常を、経過時間と次に行う動作とともに示す。実行時間上限を超過したlocal worker（`worker-timeout`。次回pollで停止し自動的に再試行キューへ戻る）、期限切れlease（`lease-expired`。次回pollで安全にqueueへ戻す）、`agent:paused`なのにこのhostのlocal workerがまだ生存している（`paused-worker-live`。次回pollの`drain_paused_workers`で停止する）、GitHub上でrunningでないlocal worker（`worker-orphan`。`worker_orphan_grace_seconds > 0`のとき、その観測が続けば次回pollで自動停止する。[ADR 0029](../decisions/0029-worker-orphan-reap.md)）、Project同期の再試行待ち（`project-sync-pending`。経過は最古の未解決entryの発生時刻基準で、後続のenqueue/ackでは伸びが途切れない）、claim一時停止中（`claim-paused`。経過は実際に成立しているpause理由のmarkerのうち最も古いもの基準）。
- **要対応**: 運用者の対応が必要な異常を、経過時間と実行可能な対応アクションとともに示す。staleなsupervisor pid/lock（`supervisor-stale-pid`。対応: `bin/agentic-loop start`）、破損したlocal state file（`local-state-corrupt`。対応: `bin/agentic-loop start`）、stall（`worker-stalled`。最後の進行がstage別のstall閾値（上記）を超過しているが上限は未超過。本文にstageと適用閾値、`worker_timeout_seconds>0`なら自動停止見込みを含む。観測のみで自動停止はしないため運用者の判断が必要）、`worker_orphan_grace_seconds=0`のときのGitHub上でrunningでないlocal worker（`worker-orphan`。自動停止が無効なため、設定変更または対象workerの手動停止が対応アクションになる）、本文が単一行のファイル参照のみのqueued Issue（`issue-body-unexpanded`。対応: `gh issue edit ISSUE --body-file <正しい本文のファイル>` で本文を再投稿し、`agent:queued` を再付与する。[ADR 0030](../decisions/0030-lost-requirement-detection.md)）。
- local stateのないrunning Issue（多端末運用では正常）と、対応するrunning Issueのない残存worktree/branch（Issue #211がpruneを担当）は、いずれもここでの異常としては通知しない。
...
`bin/agentic-loop doctor` は、`status` の稼働状況表示より広い導入・復旧向けの読み取り専用診断である。GitHub認証とrepository権限、origin/default branch、plan段・exec段・diagnoseが使用する各AI CLI（`codex`／`claude`／`opencode`、tiersを含む全providerをそれぞれ `AI CLI (<provider>)` として個別に検査）、tiersスキーマ（未知provider・空models・不正な `max_usage_percent`）、opencode go usage APIの認証keyの有無（値は表示しない。なければusage実測が使えずcooldownフォールバックになるwarning）、プール枯渇marker、Devbox、hooks、Supervisor、systemd user service/timer、Project設定、設定値、時間定数の不変条件（[ADR 0032](../decisions/0032-time-constant-invariants-and-calibration.md)。stall/provider_stall/worker_timeout/lease/pause_grace/poll/枯渇backoff/orphan_graceの相互関係、いずれもwarning）、stall閾値の実測整合（`events.log`から復元したstage別所要時間の実測p90が現在の`stall_seconds`/`provider_stall_seconds`以上ならwarning、サンプル20件未満は判定を保留してsuccess）、残存worktree/branch/logを検査し、成功・警告・失敗、影響、復旧方法を日本語で出力する。時間定数の不変条件のうち、committedな`.agentic-loop.toml`の既定値そのものの検査は`bin/agentic-loop _timing-check --committed`が`make lint`のhard gateとして別途行う（`.agentic-loop.local.toml`は無視するため、開発者のlocal overrideは他の開発者の`make lint`を落とさない）。Projectとtimerは任意の可視化・自動運用機能なので利用不能時は警告とし、GitHub Issueキュー、固定検証環境、hooks、Supervisorなど処理に必須の条件は失敗とする。
...
`.agentic-loop.toml` の `[queue]` で `poll_seconds`、`max_workers`、`lease_seconds`、`stop_timeout`、`stale_days`、`graphql_reserve`、`rate_limit_cache_seconds`、`api_retry_attempts`、`api_retry_base_seconds`、`max_attempts`、`retry_cooldown_seconds`、`worker_timeout_seconds`、`worker_orphan_grace_seconds`、`unknown_scope`、`exclusive_paths` を変更できる。個人環境向けの上書きは git 管理外の `.agentic-loop.local.toml` に同じキーを書けば、キー単位で優先される。設定はTOMLで、読み取りには `yq` を用いる。既定の並列数は4とし、これを超えてむやみに増やさない。worker失敗の多くはtoken枯渇やセッション中断などの一時的要因であるため、失敗したIssueは即座にagent:failedへ留め置かない。Supervisorの`retry_failed`が、開いている全てのagent:failed（過去分・追跡外を含む）を`[queue].max_attempts`（既定3、総試行回数）まで`[queue].retry_cooldown_seconds`（既定600秒）のクールダウンを挟んで自動的にagent:queuedへ戻して再試行し、上限に達してもcloseせず`agent:parked`（open・非claimの人間トリアージ待ち。[ADR 0016](../decisions/0016-failure-park-not-close.md)）へ移す。人手でのラベル付け替えは不要で、`resume`で再投入するかdisposeで終了するかは人間が判断する。workerが実施不要または実施不能と判断した場合は`AGENTIC_LOOP_RESULT=declined`を返すが、worker自身はcloseせず`agent:needs-input`へ載せ、認可済み運用者の判断（`dispose`等）を待つ。`[budget].max_stage_cost_usd` は1回のstage実行（plan段またはexec段の1 provider呼び出し）が使えるコストの上限（USD）で、Claudeには `--max-budget-usd` として渡る。上限に達して停止したstageはプール枯渇ではなく通常の再試行可能な失敗として扱い（プールのmarkerは書かず、claimも止めない）、Issueは既存の`max_attempts`によるbounded retryへ流れる。未設定なら上限なしで、providerへ引数を渡さない。`[budget].weekly_reserve_percent` は緊急枠の確保用で、週次利用率が `100 - 値` を超える間はSupervisorが新規Issueのclaimを一時停止し、回復すると再開する。利用率はheadlessで取得できるCodexのセッションログ（最新の `token_count` の `secondary.used_percent`）から読むbest-effortで、取得できない場合やCodex以外のproviderのみの場合はfail open（claim継続）とする。0で無効化できる。増加はCodex契約上の制限、Git競合、端末資源を確認してから行う。stopは新規claimを止め、workerをdrainする。`STOP_TIMEOUT=0` は完了まで待つ。
...
### GitHub上でrunningでないlocal workerのgrace付き自動回収（[ADR 0029](../decisions/0029-worker-orphan-reap.md)）

`recover_expired` は `agent:running` のIssueだけを、`enforce_worker_timeout` は`worker_timeout_seconds`超過だけを対象にするため、その間に空白がある: このホストのlocal worker（pidfile）は生存しているのに、GitHub上のラベルは既に `agent:running` ではない状態（worker-orphan。ラベルがqueued/failed等へ落ちたのにlocal providerプロセスが生き残った状態。実例: Issue #132）は、`worker_timeout_seconds`（既定4時間）に達するまで検出も停止もされない。Supervisorは各poll（起動時とstopのdrain待ち中を含む）で、このホストのpidfileが生存しているのにGitHub上runningでないIssueを検出し、`workers/<issue>.orphan-since` へ初回検出epochを記録する。以降のpollで引き続きorphanと判定され、かつ経過秒数が `[queue].worker_orphan_grace_seconds`（既定120秒、`0`で無効化）に達して初めて、`enforce_worker_timeout`と同じprocess-group TERM→5秒後KILLで停止し、`lease_release`/`clear_worker_local`/`scope_cache_clear`/`clear_conflict_wait`でlocal stateを掃除して監査コメント（`agentic-loop:worker-orphan-reaped`）を1件残す。Issueのラベル自体は変更しない（既に他の経路で正しい状態に設定済みという前提に立つ）。正常終了時のラベル更新からprocess終了までの短い窓で一瞬orphanに見えても、grace未満なら停止しない。grace超過後も、正常終了のcritical section（`worker_critical_active`）またはpause/abortの協調停止drain中（`worker_stop_requested`）は停止せず、`orphan-since`マーカーを保持したまま次のpollへ委ねる。同じIssueが再claimされた直後に前回workerの`orphan-since`マーカーが残っていた場合は、現worker開始epoch（`workers/<issue>.started`）より古いマーカーを無効として書き直し、graceを再計上する。running集合の取得に失敗したpollは何もしない（空集合とみなして誤検出しない）。この集合は`recover_expired`と同じpoll単位memo（`running_issue_numbers`）を共有し、poll毎に高々1回のGitHub呼び出しに留める。他host所有のIssue（このホストにpidfileが無い）には一切触れない。

### 要求本文・commentの欠落検知（[ADR 0030](../decisions/0030-lost-requirement-detection.md)）

`gh issue create --body "@path/to/file.md"` のように `--body`/`-b` に `@path` を渡すと、`--body-file`と違って展開されず、リテラル文字列 `@path/to/file.md` がそのままIssue本文・commentになり、要求が silent に失われる。この事故を3層で扱う: (1) `claim_next` はworkerへ渡す前に、本文全体が改行なしの1行で `@` から始まり空白を含まずパス様（`/`を含むか`.拡張子`で終わる）と判定できるqueued Issueを`agent:needs-input`へ移し（`mark_body_unexpanded`。冪等で、同じIssueに監査コメントを再投稿しない）、claimしない。(2) `status`は同じ判定をqueued Issueへ適用し、`issue-body-unexpanded`異常として表示する（本文そのものは出力しない）。(3) `doctor`はopen Issue本文とrepository全体の直近100件のcomment（`--paginate`なしのbest-effort）を同じ判定でスキャンし、該当があれば警告する。3層のいずれも既存の open-Issue 一覧取得・comment取得を再利用または1回追加するだけで、追加のGitHub API呼び出しは高々1回の直近comment取得であり、追加費用ゼロ。復旧は認可済み運用者が `gh issue edit ISSUE --body-file <正しい本文のファイル>` で本文を再投稿し、`agent:queued` を再付与する。closeはしない（[ADR 0016](../decisions/0016-failure-park-not-close.md)と同じ理由: 要求そのものが無効になったわけではない）。加えて、`.claude/hooks/require-gh-body-file.sh`（Claude CodeのBash PreToolUseフック）が新規のBash呼び出しレベルで同じパターンの `gh --body`/`-b` を検知し、fail-openで実行前にdenyする。
