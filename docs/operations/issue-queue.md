...
| --- | --- | --- |
| `Triage` | `is:issue is:open no:status` | Project未同期のopen Issueを検出する。CategoryまたはPriorityだけが欠けるIssueとLabel/field不整合は後述の手動検査も行う |
| `Queue` | `is:issue is:open label:"agent:queued"` | Category、Priority、Created atの順で確認する。Priorityはcritical、high、medium、low、未設定、同順位はCreated at昇順がSupervisorのclaim順である |
...
GitHub Projects APIはviewのname、layout、filterを作成・更新できる一方、CLI/APIのversionや所有者種別によってvisible field、sort、groupの更新を利用できない場合がある。現在の自動適用範囲はtable layoutとfilterまでとし、上表の「目的と表示順」をそれ以外のdesired stateとする。Project画面で列、sort、groupを上表どおり設定し、`bin/agentic-loop setup` 後に各Viewのfilterと対象集合を目視検証する。`Queue` は `bin/agentic-loop` のpriority rankとCreated atの比較、`Needs input` は `agent:needs-input` のopen Issue一覧との比較、`Recovery` の期限切れleaseは最新の `agentic-loop:lease` commentの `expires` と現在時刻の比較でdriftを検出する。
