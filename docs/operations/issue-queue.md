...
- **要対応**: 運用者の対応が必要な異常を、経過時間と実行可能な対応アクションとともに示す。staleなsupervisor pid/lock（`supervisor-stale-pid`。対応: `bin/agentic-loop start`）、破損したlocal state file（`local-state-corrupt`。対応: `bin/agentic-loop start`）、stall（`worker-stalled`。最後の進行から300秒以上経過しているが上限は未超過。観測のみで自動停止はしないため運用者の判断が必要）、`worker_orphan_grace_seconds=0`のときのGitHub上でrunningでないlocal worker（`worker-orphan`。自動停止が無効なため、設定変更または対象workerの手動停止が対応アクションになる）、本文が単一行のファイル参照のみのqueued Issue（`issue-body-unexpanded`。対応: `gh issue edit ISSUE --body-file <正しい本文のファイル>` で本文を再投稿し、`agent:queued` を再付与する。[ADR 0030](../decisions/0030-lost-requirement-detection.md)）。
