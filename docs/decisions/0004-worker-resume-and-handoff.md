...
`resume_probe()` が `worker()` の先頭でGit（`show-ref`、`rev-parse`、`worktree list`、`rev-list --left-right`、必要時のread-only `merge-tree`）とGitHub REST（`pulls?state=all&head=...`、open PR時の`pulls/<number>`、必要な場合のみ `commits/<sha>/check-runs`）だけから次のphaseを導出する。worker自身や前任workerの発言は一切参照しない。
...
| `needs-rebase` | open PRのbaseがdefault branchで、fetch済みbase側に未取り込みcommitがある、RESTの`mergeable=false`、または競合を表す`mergeable_state` | providerを起動し、`git merge origin/<default-branch>`でbaseを通常取り込み、競合解消・通常push・checks/review再確認・既存PRのmerge・default branch検証までを明示する |
...
`needs-rebase` は名前に反してrebaseを要求しない。履歴を書き換えるrebase・reset・force-pushは引き続き禁止し、baseの通常mergeだけを収束操作として許可する。GitHub RESTが非同期計算中で`mergeable=null`または`mergeable_state=unknown`なら、既に取得済みのbase/head commitへ`git merge-tree --write-tree`を実行して競合有無を補完する。この操作はworktreeを変更しない。
...
- 中断→再開1回あたりの追加GitHub REST呼び出しは、branch未作成またはdefault branchと同一の場合0、それ以外で最大2（PR一覧、open PRがある場合はPR詳細とcheck-runsを加え最大3）。GraphQLはこの判定に使わない。
