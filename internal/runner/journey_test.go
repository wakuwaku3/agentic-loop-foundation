...
func TestJourneyOneLocalIssueIntakeToIncrementArtifact(t *testing.T) {
	...
	planResp, err := service.Plan(owner, application.PlanRequest{RequestID: "journey1:plan", RequirementID: capResp.RequirementID, ExpectedRequirementVersion: capResp.Version})
	if err != nil {
		t.Fatalf("plan: %v", err)
	}
