...
# Machine-generated changes that sync may safely fast-forward across: a
# tracked .agentic-loop/manifest.json rewritten by install/upgrade to record
# the applied revision, and a not-yet-committed .agentic-loop/capabilities.toml
# freshly generated by install/upgrade (see docs/decisions/0018-repository-
# capability-manifest.md). A fast-forward never touches either an untracked
# file or the working tree outside the merge, so tolerating them here cannot
# lose or overwrite anything; the operator still has to commit the capability
# manifest themselves (doctor warns until they do). Prints every other dirty
# change (empty means the only dirt is one or both of these). Any user or
# Foundation content is refused, exactly as before this tolerance existed.
dirty_beyond_manifest() {
  git -C "$1" status --porcelain | awk '
    $0 == " M .agentic-loop/manifest.json" { next }
    $0 == "?? .agentic-loop/capabilities.toml" { next }
    { print }
  '
}
