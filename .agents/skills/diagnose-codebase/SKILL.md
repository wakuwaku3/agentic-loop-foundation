...
8. Create one Japanese GitHub Issue per independently actionable finding with the `diagnosis` label. Include the problem, evidence, expected state, suggested acceptance criteria, and affected paths. Do not add `agent:queued`; diagnosis reports work but does not authorize a fix.
9. If no actionable finding exists, create no Issue and report that result.

Keep Issue titles, bodies, and comments in Japanese. Never commit, edit, delete, push, open a pull request, or change Issue state/labels other than creating the diagnosis Issue itself.
