...
インストールは対象リポジトリ専用のLabelsとGitHub Projectを冪等に設定し、Supervisorをバックグラウンド起動します。また、リポジトリごとのuser-level systemd timerを有効化し、15分間隔（最大2分のランダム遅延あり）でローカルの`main` worktreeを`origin/main`へ追従させます。更新はcleanな`main`に対するfast-forwardだけに限定され、ローカル変更や分岐があれば何も変更せず失敗します。登録状態は`systemctl --user list-timers 'agentic-loop-main-sync-*'`、実行履歴は`journalctl --user -u 'agentic-loop-main-sync-*'`で確認できます。
