...
rg -n 'name\s*=\s*"(?!\(default\))[^" ]+"' "$infra" --pcre2 >/dev/null && fail 'named Firestore databases are forbidden'
rg -n 'name\s*=\s*"\(default\)"' "$infra" >/dev/null || fail 'default Firestore database is missing'
