...
main() {
  ...
  chmod +x "$target/bin/agentic-loop" "$target/bin/agentic-loop-diagnose" "$target/.agentic-loop/guard-secrets.sh" "$target/.agentic-loop/update-main.sh" "$target/.agentic-loop/diagnose-codebase.sh" "$target/.githooks/pre-commit" "$target/.githooks/pre-push" "$target/.claude/hooks/confirm-main-worktree-edit.sh" "$target/.claude/hooks/require-gh-body-file.sh"
