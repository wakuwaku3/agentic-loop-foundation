    ...
    if [[ $* == *'projectItems(first:20'* ]]; then
      ...
      if grep -Eq "/(issues|pull)/$number$" "$project_items" 2>/dev/null; then
        ...
        if [[ -r $project_page2 ]] && grep -Fxq "$number" "$project_page2" && [[ -z $cursor ]]; then
          ...
          printf '{"data":{"repository":{"content":{"projectItems":{"nodes":[{"id":"PVTI_%s","project":{"id":"PVT_fixture"},"fieldValues":{"nodes":[{"name":"%s","field":{"name":"Agent status"}},{"name":"%s","field":{"name":"Category"}},{"text":"%s","field":{"name":"Blocked by"}}]}}],"pageInfo":{"hasNextPage":false,"endCursor":null}}}}}\n' "$number" "$status" "$category" "$blocked" | jq -r "$jq_expr"
        fi
      else
