...
grep -Fq 'worker-orphan' <<< "$status_github_failure_output" && fail 'status falsely reported worker-orphan during a GitHub failure'
grep -Fq 'residual-worktree' <<< "$status_github_failure_output" && fail 'status falsely reported residual-worktree during a GitHub failure'
